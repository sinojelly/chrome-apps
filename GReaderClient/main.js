...
var IMPORTANT_LIST_RE_ =	
	new RegExp('user/[\\d]+/label/Important');

var LABLE_LIST_RE_ =	
	new RegExp('user/[\\d]+/label/');
...
function updateUnreadCount(importantCount, totalCount) {
  chrome.browserAction.setIcon({path: 'icon-signed-in.png'});
  var countText = '';
  if (totalCount > 0) {
    countText = importantCount + '';
  }
  ...
  chrome.browserAction.setTitle({title:countText + '/' + totalCount});
  badgeFadeAnimation =
...
function startRequest(opt_noSchedule) {
  ...
  getUnreadCount(
    function(importantCount, totalCount) {
      loadingAnimation.stop();
      updateUnreadCount(importantCount, totalCount);
      if (!opt_noSchedule) {
        scheduleRequest();
      }
    },
...
function getUnreadCount(onSuccess, onError) {
  ...
  function handleSuccess(jsonText) {
    ...
    for (var i = 0, stream; stream = json.unreadcounts[i]; i++) {
	  if (IMPORTANT_LIST_RE_.test(stream.id)) {
	    importantCount = stream.count;
		continue;
	  }
	  
	  // sinojelly: add all feed's count sometimes still not correct, because some feed's count larger than 1000, it return 1000+.
	  // but this is not important.
	  if (FEED_RE_.test(stream.id)) { 
		totalCount += stream.count;
	  }
    }
	
	onSuccess(importantCount, totalCount);
  }
